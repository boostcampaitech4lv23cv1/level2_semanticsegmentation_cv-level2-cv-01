...
output_list = os.listdir("./hard_vote_ensemble")
...
for output in output_list:
    df_list.append(pd.read_csv(f"./hard_vote_ensemble/{output}"))
...
for idx in tqdm(range(len(df_list[0]))):
    ...
    for i in range(len(pixel_list[0])):
        ...
        voted_pixel = [
            ...
            if value == max(pixel_count.values())
        ]
...
submission.to_csv("./12csvs_withouPL.csv", index=False)
